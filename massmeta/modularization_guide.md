...
⚠️ Вся работа с любыми картами идёт **Не в модульной папке**!

Все наши Новые карты лежат по пути **`_maps/map_files/`**

К каждой карте идёт дополнительно **`.json`** файл-конфигурации в **`_maps/`**, не забудьте добавить его тоже!

Также впишите новую карту в **`_maps/_basemap.dm`** (под комментами), чтобы её тоже скомпилировало.
